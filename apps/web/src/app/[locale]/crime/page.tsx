...
import { getAllCrime, getCities, getHeadModel, getJustice } from '@/data/loaders';
...
export default async function CrimePage({
  ...
  const headModel = getHeadModel();
  // Disposal is state-level; only Bengaluru/Karnataka is populated for now.
  const justiceByCity: Record<string, JusticeData | null> = { bengaluru: getJustice('bengaluru') };

  return (
    <div className="flex min-h-screen flex-col">
      <SiteHeader showCity />
      <main id="main" className="mx-auto w-full max-w-[1100px] flex-1 px-5 py-8 sm:px-8">
        <Suspense fallback={<div className="py-20 text-center text-ink-faint">Loading…</div>}>
          <CrimeDashboard allCrime={allCrime} headModel={headModel} cities={cities} justiceByCity={justiceByCity} />
        </Suspense>
      </main>
      <TrustFooter />
    </div>
